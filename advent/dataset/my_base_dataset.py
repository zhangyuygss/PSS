...
class BaseDataSet(Dataset):
    def __init__(self, data_dir, split, mean, std, ignore_index, list_file,
                base_size=None, augment=True, val=False,
                jitter=False, use_weak_lables=False, weak_labels_output=None, 
                crop_size=None, scale=False, flip=False, rotate=False,
                blur=False, return_id=False, n_labeled_examples=None,
                random_seed=None, batch_size=None):
    ...
    def _crop(self, image, label, center_crop=False):   
    ...
    def _val_augmentation(self, image, label):
        if self.base_size is not None:
            image, label = self._resize(image, label, bigger_side_to_base_size=False)
            image, label = self._crop(image, label, center_crop=True)
    ...
    def __getitem__(self, index):
        image, label, image_id, im_sz =  self._load_data(index)
        if self.val:
        ...
        # print(image_id)
        return image, label, image_id, '-', im_sz
